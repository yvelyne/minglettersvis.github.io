...
function draw_graph(containerid, data, save_layout) {
    ...
    function neighboring(id1, id2) {
        return linkedByIndex[id1 + "," + id2];
    }
    ...
    function show_connected(node_) {
        ...
        d3.selectAll('.node')
            .style("opacity", 0)
            .filter((d, i) => filter_node_by_year(d.id))
            .style("opacity", function (o) {
                if (!neighboring(node_.id, o.id) && !neighboring(o.id, node_.id)) {  // 不相连
                    return 0.3;
                } else { // 相连
                    return 1;
                }
        ...
        d3.selectAll('.person_label')
            ...
            .filter(function (o) {
                if (!neighboring(node_.id, o.id) && !neighboring(o.id, node_.id) && node_.id!==o.id) {  // 不相连
                    return 0;
                } else { // 相连
                    return 1;
                }
    ...
    simulation.on("tick", () => {
        ...
        label.attr("x", d => {
                return d3.select("#node" + d.id)._groups[0][0].__data__.fx;
            })
            .attr("y", d => {
                return d3.select("#node" + d.id)._groups[0][0].__data__.fy-5;
            })
    })
        .on("end", function () {
